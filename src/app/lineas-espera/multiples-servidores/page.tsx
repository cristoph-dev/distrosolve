...
import {
  ...
  calculateMultipleServerUnlimited,
  MultipleServerQueueMetrics,
  MultipleServerQueueModel,
} from "@/lib/multiple-server-queues";
...
export function MultipleServersQueueModule({ embedded = false }: { embedded?: boolean } = {}) {
  useEffect(() => {
    document.title = "Líneas de espera de múltiples servidores | distrosolve";
  }, []);

  const [model, setModel] = useSessionState<MultipleServerQueueModel>(
    "multiple-server-queue:model",
    "mms"
  );
  const [lambda, setLambda] = useSessionState("multiple-server-queue:lambda", 10);
  const [serviceRate, setServiceRate] = useSessionState("multiple-server-queue:serviceRate", 6);
  const [servers, setServers] = useSessionState("multiple-server-queue:servers", 3);
  const [capacity, setCapacity] = useSessionState("multiple-server-queue:capacity", 12);
  const [results, setResults] = useSessionState<MultipleServerQueueMetrics | null>(
    "multiple-server-queue:results",
    null
  );
  const [error, setError] = useSessionState<string | null>("multiple-server-queue:error", null);
  const [hasCalculated, setHasCalculated] = useSessionState(
    "multiple-server-queue:hasCalculated",
    false
  );
  ...
  const calculate = () => {
    ...
    const output = model === "mms"
      ? calculateMultipleServerUnlimited(lambda, serviceRate, servers)
      : calculateMultipleServerLimited(lambda, serviceRate, servers, capacity);
    ...
    <div className={cn("mx-auto w-full max-w-[1120px] space-y-6", embedded ? "" : "p-4 pb-24 md:p-6")}>
      ...
      <div className="space-y-3">
        ...
        <div className="relative flex h-10 w-full max-w-md overflow-hidden rounded-lg border border-zinc-800 bg-zinc-950 p-1">
          <span className="pointer-events-none absolute inset-y-1 left-1 w-[calc(50%-0.25rem)] translate-x-full rounded-md bg-zinc-800 transition-transform duration-300 ease-out" />
          <Link
      ...
      <div className={cn(
          ...
          <Card className="relative rounded-xl border-zinc-800 bg-zinc-900 text-white">
            <div className="absolute top-3 right-3">
              <Popover>
                ...
                <PopoverContent side="top" align="end" className="w-80 bg-zinc-900 border-zinc-800 text-zinc-300">
                  <div className="space-y-3">
                    <h4 className="font-mono text-xs uppercase tracking-wider text-white">Línea de espera de múltiples servidores</h4>
                    <p className="popover-copy">
                      Modelo M/M/s o M/M/s/K para sistemas con varios canales de atención operando en paralelo.
                    </p>
                    <GlossaryPopoverLink href="/glossary#multiples-servidores" />
                  </div>
                </PopoverContent>
              </Popover>
            </div>
            <CardHeader>
              ...
              <CardDescription className="card-description-copy">
                Define las tasas, los servidores y la capacidad si existe límite.
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-5">
              <Tabs
                value={model}
                onValueChange={(value) => setModel(value as MultipleServerQueueModel)}
              >
              ...
              <NumberInput
                ...
                min={2}
                step={1}
                onChange={(value) => setServers(Math.max(2, Math.trunc(value)))}
              />

              <div className={cn("transition-all", model === "mmsk" ? "block" : "hidden")}>
                <NumberInput
                  label="Capacidad del sistema (K)"
                  value={capacity}
                  min={servers}
                  step={1}
                  onChange={(value) => setCapacity(Math.max(servers, Math.trunc(value)))}
                />
              </div>
        ...
        <Card className={cn(
          ...
          <CardHeader className="border-b border-zinc-800/50">
            ...
            <CardDescription className="card-description-copy">
              Indicadores del sistema multiserver y distribución P(n).
            </CardDescription>
          </CardHeader>
            ...
            {!hasCalculated || !results ? (
              <div className="flex h-full flex-1 items-center justify-center p-6 text-center">
                <p className="max-w-sm text-[13px] leading-5 text-zinc-500">
                  Ejecuta el cálculo para visualizar utilización, tiempos, promedios y probabilidades.
                </p>
              </div>
            ) : (
...
export default function MultipleServersQueuePage() {
  ...
  return <MultipleServersQueueModule embedded />;
}
